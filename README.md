...
- `ListenMark-0.1.3-arm64.dmg`
- `ListenMark-0.1.3-arm64.zip`
...
| **留档** | 原文 + 动作 + 来源 + 时间 + AI 回应 → 本地，可手动或自动保存 | 无 |
| **档案** | 时间线 + 全文搜索 + 一键重听 | 无 |
...
2. 菜单栏耳朵图标 → **设置...**：
   ...
   - **语音合成引擎**：默认选择火山引擎；填 App ID / Access Token / 音色后使用豆包语音，未配置时自动回退本地。

## 用法

- 任意应用里选中文字 → 面板自动弹出（或按弹出面板快捷键，默认 `Option + Command + R`）。
- 无法取词时，可在设置里配置屏幕选框 OCR 快捷键；默认 `Control + Shift + O`，框选区域识别后进入同一个处理面板。
- 面板上的复制图标会立即复制，并弹出轻量气泡让你顺手留档。
- 如果本次 AI 回答附带了上下文，结果标题旁会显示「已附带上下文」；留档时只保存选中内容上下各 200 字的上下文摘录，并标记选中内容。
...
- 文本动作走 DeepSeek SSE 流式：边出字，完整结果生成后再朗读。
