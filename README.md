...
<p align="center">
  <img src="docs/workflow.png" alt="Dob 工作流：选中文本 → 朗读听 → 结合上下文与 AI 处理 → 留进资料库" width="820" />
</p>

---

**Dob** 是一款 macOS 原生菜单栏 App。它围绕一个很小的动作展开：选中一段文字，先用朗读把它听懂，把值得记住的留档存进本地资料库，再按需解释、翻译、审校或比较，让重要的内容日后还能回到你眼前。
...
打开 Releases 页面，下载最新的 `Dob-...-arm64.dmg`。

安装包已签名并通过 Apple 公证，Dob 会自动检查更新。界面语言默认跟随系统中 / 英，也可在「设置 → 通用」里切换。
...
没有 API Key 时，仍可使用朗读、留档、OCR、复制、历史和档案。AI 技能需要 OpenAI 兼容 Chat Completions API。
...
<p align="center">
  <img src="docs/toolbar.png" alt="选中文本后，屏幕边缘弹出的紧凑工具条" width="760" />
</p>
...
留档是 Dob 的另一半：朗读让信息经过你的大脑，留档让它留在你的电脑。任何一次结果都可以一键写入本地资料库，日后复习。
...
<p align="center">
  <img src="docs/archive.png" alt="留档记录：原文、结果、来源链接与上下文摘录" width="640" />
</p>
...
<p align="center">
  <img src="docs/context.png" alt="解释结果结合整段上下文，并标注「已附带上下文」" width="760" />
</p>
...
<p align="center">
  <img src="docs/compare.png" alt="同一个技能交给多个模型，结果上下分栏比较" width="760" />
</p>
...
如果你已经长期使用 PopClip，可以从 Releases 下载 `Dob-PopClip-...popclipextz`，或安装仓库里的 `PopClip/Dob.popclipext`，把 Dob 的「朗读、解释、翻译、提炼、留档」放进 PopClip 工具条。

安装后建议在 Dob 设置中关闭「划词后自动弹出」，这样选中文字时只出现 PopClip；点击 PopClip 里的 Dob 动作后，再由 Dob 执行朗读、解释或留档。
